...
#The rest of the code handles deleting users who are no longer in the main campus address book.
if ($DeleteOnly) {
	Write-Output "Begining to delete invalid contacts"
}
else {
	Write-Output "Import Done - Deleting invalid contacts"
}

try {
	"Reading main campus addresses"
	$ValidMCContacts = Import-Csv $ImportFile -ErrorAction Stop
}
catch {
	Write-Warning "Unable to read main campus import file"
	Invoke-HSCExitCommand -ErrorCount $Error.Count
}

Write-Output "Generating list of current contacts in the HS domain"
$AzureADContacts = Get-AzureADContact -All |
	Where-Object {($_.EmailAddress -like "*@mail.wvu.edu") -OR
					($_.EmailAddress -like "*@wvu.edu") }

$DeleteCount = 0
$TotalCount = 0

#Begin searing for accounts to delete
foreach ($AzureADContact in $AzureADContacts)
{
	$TotalCount++

	Write-Output $("Processing: " + $AzureADContact.EmailAddress)

	if ($ValidMCContacts.PrimarySMTPAddress -contains $AzureADContact.Mail)
	{
		Write-Output $("Found: " + $ValidMCContact.EmailAddress)
	}
	else
	{
		$DeleteCount++
		Write-Output "Total Deleted Contacts: $DeleteCount"
		Write-Output $("Not Found: " + $contact.EmailAddress)

		#Get-AzureADContact -SearchString $contact.EmailAddress |
			#Remove-AzureADContact -force

		Add-Content -Path $DeletedAccounts -Value $AzureADContact.EmailAddress
	}

	Write-Output "Total Count: $TotalCount"

	#The following if statement is a safeguard to ensure that all contacts aren't deleted.
	#Usually less than 10 are deleted.
	if ($DeleteCount -gt $MaximumDeletes)
	{
		#A final safeguard to prevent accounts from being deleted.
		Write-Output "$MaximumDeletes accounts have been deleted."
...
if (!$DeleteOnly)
{
	Write-Output "`n`nAccount Creation & Update Summary"
	Write-Output "New Contacts Created: $NewContact"
	Write-Output "Existing Contacts: $ExistingContacts"
}

Write-Output "`n`nAccound Deletion Summary"
Write-Output "Total Processed: $TotalCount"
Write-Output "Deleted:  $DeleteCount"
