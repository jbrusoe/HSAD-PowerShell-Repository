<#
	...
	.DESCRIPTION
		...
		Functions used for establishing connection to Office 365 (AzureAD/MSOL)
		...
		Functions for AzureAD/MSOnline (* = Function to be implemented)
...
#########################################################################
# Functions related to establishing Office 365 Connection(AzureAD/MSOL) #
#########################################################################
...
function Connect-HSCOffice365
{
	<#
		...
		.PARAMETER Account
			A null value for this uses Get-HSCConnectionAccount to build the
            credential object. Otherwise, the credential object is built using
		...
		.EXAMPLE
			...
			Encrypted File Path: C:\Users\jbrusoeadmin\Documents\GitHub\...\jbrusoeadmin-HSVDIWIN10JB.txt
	...
	Write-Output "Attempting to Connect to Office 365...`n"
	...
	$TestHSConnectionRequirementParams = @{
		Account = $Account
		EncryptedFilePath = $EncryptedFilePath
		MaxPowerShellVersion = $MaxPowerShellVersion
		ErrorAction = "Stop"
	}

	try {
		Write-Verbose "Testing Office 365 AzureAD/MSOL Connection Requirements"

		if (Test-HSCConnectionRequirement @TestHSConnectionRequirementParams) {
			Write-Verbose "Connection Account: $Account"
			Write-Verbose "Encrypted File Path: $EncryptedFilePath"
			Write-Verbose "Connecting to the HSC Office 365 tenant with AzureAD cmdlets..."
		}
		else {
			Write-Warning "Connection prequisites not met"
			$AttemptConnection = $false
		}
	}
	catch {
		Write-Warning "Office 365 connection requirements not met"
		$AttemptConnection = $false
	}
    ...
    if ($AttemptConnection) {
        try {
            ...
            $Password = Get-Content $EncryptedFilePath -ErrorAction Stop | ConvertTo-SecureString
            ...
            $Credential = New-Object @NewObjectParams
    
            Write-Verbose "Successfully generated credential object"
        }
        catch {
            Write-Warning "Error generating credential. Unable to connect to Office 365 with AzureAD"
            $AttemptConnection = $false
        }    
    }
    ...
    if ($AttemptConnection) {
        try {
            ...
            Write-Output "Authenticated to Office 365`n"
        }
        catch {
            Write-Warning "Unable to authenticate to the Office 365 tenant with AzureAD cmdlets"
        }
    }
    else {
        Write-Output "Unable to authenticate to Office 365"
    }
}
...
function Connect-HSCOffice365MSOL
{
    ...
    try {
        ...
        Write-Warning "Unable to determine connection account"
    }
    
    if ($MSOLConnected) {
        Write-Verbose "Determining path to encrypted file"

        try {
            if ($UseSysscriptDefault) {
                $EncryptedFilePath = Get-HSCEncryptedFilePath -UseSysscriptDefault -ErrorAction Stop
            }
            else {
            ...
            Write-Warning "Unable to determine path to encrypted file"
            $MSOLConnected = $false
        }
    }
    ...
    if ($MSOLConnected)
    {
        Write-Verbose "Checking MSOL connection requirements..."
        ...
        try {
            ...
            if (Test-HSCConnectionRequirement @TestHSConnectionRequirementParams) {
                Write-Verbose "Connection Account: $Account"
                Write-Verbose "Encrypted File Path: $EncryptedFilePath"
                Write-Verbose "Connecting to the HSC Office 365 tenant with MSOL cmdlets..."
            }
            else {
                Write-Warning "Connection prequisites not met"
                $MSOLConnected = $false
            ...
            Write-Verbose "MSOLConnection requirements met"
        }
        catch {
            Write-Warning "Unable to determine if connection prerequisites are met"
            $MSOLConnected = $false
        }
    }
    ...
    if ($MSOLConnected) {
        try {
            ...
            Write-Warning "Error generating credential. Unable to connect to Office 365 with MSOnline cmdlets"
            $MSOLConnected = $false
        }
    }
    ...
    if ($MSOLConnected){
        try {
            ...
            Write-Output "Authenticated to Office 365 with MSOL cmdlets`n"
        }
        catch {
            Write-Warning "Unable to authenticate to the Office 365 tenant with MSOL cmdlets`n"
			$MSOLConnected = $false
        }
    }
	
	return $MSOLConnected
}
...
function Connect-HSCExchangeOnlineV1
{
	...
	process
	{
		...
		try 
		{
			...
			Write-Verbose "`nSuccessfully authenticated to Exchange Online and downloadeded PowerShell V1 cmdlets`n"
			#return $true
...
function Get-HSCO365UserLicense
{
	<#
		.SYNOPSIS
	...
	process
	{
		...
		{
			Write-Verbose "Getting license information for: $UserName"
			...
			try 
			{
				...
				if (($AzureADUser | Measure-Object).Count -eq 0) {
					Write-Warning "Unable to find AzureAD user"
					Continue
				}
				elseif (($AzureADUser | Measure-Object).Count -gt 1) {
					Write-Warning "Unable to find one unique user"
					Continue
				}
				else {
				...
				Write-Warning "Unable to find AzureAD user: $UserName"
				return $null
			}
	
			#Now find user license info
			try {
				$UserLicenses = Get-AzureADUserLicenseDetail -ObjectId $AzureADUser.ObjectId |
					Select-Object SkuPartNumber
				
				Write-Verbose "Successfully generated user licenses"
...
function Remove-HSCAzureADEMSLicense 
{
	<#
		.SYNOPSIS
			This function is used to remove AzureAD licenses.
			
		.PARAMETER SamAccountName
			The SamAccountName of the user to remove the license from.

		.PARAMETER LicenseName
			The name of the license to remove.
	...
	process
	{
		...
		{
			try {
				Write-Verbose "Attempting to find Azure AD User"
				$AzureADUser = Get-AzureADUser -SearchString $($SAM + "@hsc.wvu.edu") -ErrorAction Stop
		
				if ($null -eq $AzureADUser) {
					throw
				}
				else {
					Write-Verbose "Successfully found AzureADUser"
				}
			}
			catch {
				try {
					Write-Verbose "Second try"
					$AzureADUser = Get-AzureADUser -SearchString $SAM -ErrorAction Stop
					...
					Write-Warning "Unable to find AzureADuser"
				}
			}

			$UPN = $AzureADUser | Select-Object UserPrincipalName
			$UPNToRemove = ($UPN | Where-Object {$_.UserPrincipalName -like "*hsc.wvu.edu*"}).UserPrincipalName
			Write-Verbose "UPN: $UPNToRemove"
		
			$LicenseToRemove = New-Object -TypeName Microsoft.Open.AzureAD.Model.AssignedLicenses
			$LicenseToRemove.RemoveLicenses = (Get-AzureADSubscribedSku | Where-Object {$_.SkuPartNumber -eq $LicenseName}).SkuID
		
			try {
				Set-AzureADUserLicense -ObjectId $UPNToRemove -AssignedLicenses $LicenseToRemove -ErrorAction Stop
				Write-Host "Successfully removed license" -ForegroundColor Green
			}
			catch {
				Write-Warning "Unable to remove user license"
			}
...
function Remove-HSCAzureADO365A3License 
{
	<#
		.SYNOPSIS
			This function is used to remove AzureAD licenses.
	...
	param (
		...
		[ValidateNotNullOrEmpty()]
		[string]$LicenseName="ENTERPRISEPACKPLUS_FACULTY"
	)
...
Function Get-HSCO365MailboxStatus 
{
	...
	[OutputType([PSObject])]
	param (
		[string]$ExportFile = $($MyInvocation.PSScriptRoot + "\O365MailboxStatus.csv")
	)
	
    if (Test-Path $ExportFile) 
    {
        Write-Verbose "Cleaning Up Old Export File" | Out-Host
        Remove-Item -Path $ExportFile -Force
    }

    Write-Verbose "Getting Mailboxes in Office 365" | Out-Host
    $users = get-casmailbox -resultsize unlimited | Where { $_.PrimarySMTPAddress -notlike "*rni.*" -OR $_.PrimarySMTPAddress -notlike "*wvurni" }

    [PSObject[]]$MailboxStatus = @()
    [PSObject[]]$O365Enabled = @()

    Write-Verbose "Getting information for users with SIP addresses...." | Out-Host
    foreach ($user in $users)
    {
        $userArray = New-Object -TypeName psobject
        
		if ($user.emailaddresses -clike "SMTP:*")
        {
            $usersmtp = $user.EmailAddresses -clike "SMTP:*" | Select-String 'SMTP:'
            $usersmtp = $usersmtp -replace "SMTP:"
            $usersmtp = $usersmtp.ToLower()


            $userArray | Add-Member -Name "O365EmailAddress" -Value $usersmtp -MemberType NoteProperty
            $userArray | Add-Member -Name "OWAEnabled" -Value $user.OWAEnabled -MemberType NoteProperty
            $userArray | Add-Member -Name "MAPIEnabled" -Value $user.MAPIEnabled -MemberType NoteProperty
            $userArray | Add-Member -Name "ActiveSyncEnabled" -Value $user.ActiveSyncEnabled -MemberType NoteProperty
            $MailboxStatus += $userArray
			
            if (($userArray.OWAEnabled -eq $true) -and ($userArray.MAPIEnabled -eq $true) -and ($userArray.ActiveSyncEnabled -eq $true))
            {
                $O365Enabled += $userArray
            }
        
        }
        elseif ($user.emailaddresses -clike "SIP:*")
        {
            $usersip = $user.EmailAddresses | Select-String 'sip:'
            $usersip = $usersip -replace "sip:"
            $usersip = $usersip.ToLower()


            $userArray | Add-Member -Name "O365EmailAddress" -Value $usersip -MemberType NoteProperty
            $userArray | Add-Member -Name "OWAEnabled" -Value $user.OWAEnabled -MemberType NoteProperty
            $userArray | Add-Member -Name "MAPIEnabled" -Value $user.MAPIEnabled -MemberType NoteProperty
            $userArray | Add-Member -Name "ActiveSyncEnabled" -Value $user.ActiveSyncEnabled -MemberType NoteProperty
            ...
            if (($userArray.OWAEnabled -eq $true) -AND
				($userArray.MAPIEnabled -eq $true) -AND
				($userArray.ActiveSyncEnabled -eq $true)) {
                $O365Enabled += $userArray
            }
        }
    }
...
function Get-HSCRecoverableItemStatistics {
	<#
		.SYNOPSIS
			This function gives the statistics for a particular user's
			recoverable items in Office 365.

		.OUTPUTS
			PSObject

		.PARAMETER Identity
			The mailbox that will have its recoverable items information returned

		.EXAMPLE
			Get-HSCRecoverableItemStatistics jbrusoe@hsc.wvu.edu

			Name              FolderAndSubfolderSize       ItemsInFolderAndSubfolders
			----              ----------------------       --------------------------
			Recoverable Items 194.6 MB (204,098,551 bytes)                       5934
			Audits            8.405 MB (8,813,606 bytes)                         1497
			Calendar Logging  42.06 MB (44,101,392 bytes)                         316
			Deletions         15.82 MB (16,584,614 bytes)                         147
			DiscoveryHolds    25.74 KB (26,354 bytes)                              22
			Purges            0 B (0 bytes)                                         0
			SubstrateHolds    128.3 MB (134,572,585 bytes)                       3952
			Versions          0 B (0 bytes)                                         0

		.EXAMPLE
			"jbrusoe@hsc.wvu.edu" | Get-HSCRecoverableItemStatistics -Verbose

			VERBOSE: User being searched for: jbrusoe@hsc.wvu.edu
	...
	Write-Verbose "User being searched for: $UserName"

	try {
		$GetMailboxFolderParams = @{
			Identity = $Identity
		...
		$Folders = Get-MailboxFolderStatistics @GetMailboxFolderParams
		
		foreach ($Folder in $Folders) {
			$Folder | Select-Object $Properties
		}
	}
	catch {
		Write-Warning "Unable to display recoverable item statistics for $Identity"
		return $null
	}
}

function Get-HSCMailboxClutterStatus
{
	<#
		.SYNOPSIS
			This function determines the clutter status of a mailbox(es)

		.OUTPUTS
			PSObject

		.PARAMETER PrimarySMTPAddresses
			This the email address(es) that will have it's clutter status returned
		
		.EXAMPLE
			Get-HSCMailboxClutterStatus jbrusoe

			PrimarySMTPAddress ClutterEnabled UserFound
			------------------ -------------- ---------
			jbrusoe                      True      True

		.EXAMPLE
			Get-HSCMailboxClutterStatus jbrusoe,krussell,asdf
		...
		.EXAMPLE
			Get-HSCMailboxClutterStatus jbrusoe,krussell -Verbose
			VERBOSE: Current Primary SMTP Address: jbrusoe
			VERBOSE: Attempting to find mailbox
			VERBOSE: Getting clutter information

			VERBOSE: Current Primary SMTP Address: krussell
			VERBOSE: Attempting to find mailbox
			VERBOSE: Getting clutter information
			PrimarySMTPAddress ClutterEnabled UserFound
			------------------ -------------- ---------
		...
		.EXAMPLE
			"jbrusoe","krussell" | Get-HSCMailboxClutterStatus
	...
	param(
		[Parameter(Mandatory = $true,
					...
					ValueFromPipelineByPropertyName = $true,
					Position = 0)]
		[string[]]$PrimarySMTPAddresses
	)

	process
	{
		foreach ($PrimarySMTPAddress in $PrimarySMTPAddresses) {
			Write-Verbose "Current Primary SMTP Address: $PrimarySMTPAddress"

			try {
				Write-Verbose "Attempting to find mailbox"
				$O365Mailbox = Get-Mailbox -Identity $PrimarySMTPAddress -ErrorAction Stop

				$UserFound = $true
			}
			catch {
				Write-Verbose "Unable to find user"
				$UserFound = $false
			}
			...
			if ($null -eq $O365Mailbox) {
				...
				try {
					Write-Verbose "Getting clutter information"
					$ClutterInfo = Get-Clutter -Identity $O365Mailbox.PrimarySMTPAddress -ErrorAction Stop
				}
				catch {
					Write-Warning "Unable to pull clutter info"
					$ClutterInfo = $null
				}
			}

			$ClutterInfoObject = [PSCustomObject]@{
				PrimarySMTPAddress = $PrimarySMTPAddress
				ClutterEnabled = $ClutterInfo.IsEnabled
				UserFound = $UserFound
			}
