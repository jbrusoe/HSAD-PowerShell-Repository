...
######################################################################################################################################
#Retrieve and load XML feed of "CANCER CELL BIOLOGY GRADUATE EDUCATION (MBRCC)" from HSC Directory
$url = New-Object XML
$url.load("https://sole.hsc.wvu.edu/Apps/Directory/XMLView.aspx?DepartmentID=2075")
$users = @()
...
$output = "Loading " + $users.count + " users from the CANCER CELL BIOLOGY GRADUATE EDUCATION (MBRCC) directory webpage"
#####echo " "
#echo $output
foreach ($i in $users){
