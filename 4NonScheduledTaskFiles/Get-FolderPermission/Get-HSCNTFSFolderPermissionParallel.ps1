...
#Function Get-HSCNTFSFolderPermission {
    ...
    if(![string]::IsNullOrEmpty($SavePath)){
        ...
        $LoggedInUser = (Get-CimInstance -class Win32_ComputerSystem | Select-Object username).username
    ...
    if($FolderCount -eq 1){        
        ...
        foreach ($Member in $ACL.Access) {                
            ...
            $GroupOrUser = Get-UserOrGroup -UserName $CheckGroupOrUser.Line.Split('\')[1]

            if ($GroupOrUser -eq "user") {
                try{
                    Write-Output "Checking User: $($CheckGroupOrUser.Line.Split('\')[1])"
                    
                    if (Get-ADUser $CheckGroupOrUser.Line.Split('\')[1]){

                        $FindUserDept = Get-ADUser $CheckGroupOrUser.Line.Split('\')[1] -Properties Department, Mail, CN                        
                    ...
                    Write-Warning "There was an error getting information on $($CheckGroupOrUser.Line.Split('\')[1])"
                }
            }

            if ($GroupOrUser -eq "group") {
                try{
                    Write-Output "Checking Group: $($CheckGroupOrUser.Line.Split('\')[1])"

                    if (Get-ADGroup $CheckGroupOrUser.Line.Split('\')[1]){
                        if (($CheckGroupOrUser.Line.Split('\')[0] -eq 'BUILTIN') -OR ($CheckGroupOrUser.Line.Split('\')[0] -eq 'NT AUTHORITY')) {
                        }
                        else{                            
                            [PSCustomObject]@{
                                            'Path' = "$FolderPath";
    ...
    if($FolderCount -gt 1) {    
        Get-ChildItem -Path $FolderPath -Recurse -ErrorAction SilentlyContinue -Directory | ForEach-Object -Parallel {
            ...
            foreach ($Member in $ACL.Access) {    
                ...
                $RemovedDomainUserName = Split-Path -Path $CheckGroupOrUser -Leaf        
                try {                    
                    $GroupOrUser = Get-UserOrGroup -UserName $RemovedDomainUserName -ErrorAction Stop                    
                }
                catch {
                ...
                if ($GroupOrUser -eq "user") {
                    try{
                        Write-Output "Checking User: $($RemovedDomainUserName)"
                        
                        if (Get-ADUser $RemovedDomainUserName){

                            $FindUserDept = Get-ADUser $RemovedDomainUserName -Properties Department, Mail, CN
                        ...
                        Write-Warning "There was an error getting information on $($RemovedDomainUserName)"
                    }
                }

                if ($GroupOrUser -eq "group") {
                    try{
                        Write-Output "Checking Group: $($RemovedDomainUserName)"

                        if (Get-ADGroup $RemovedDomainUserName){
                            if (($RemovedDomainUserName -eq 'BUILTIN') -OR ($RemovedDomainUserName -eq 'NT AUTHORITY')) {
                            }
                            else{
                                [PSCustomObject]@{
                                                'Path' = $_.FullName
