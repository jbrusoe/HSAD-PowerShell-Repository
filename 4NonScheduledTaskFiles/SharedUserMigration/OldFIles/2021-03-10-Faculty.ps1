$Residents = Import-Csv 2021-03-10-Faculty.csv
[string]$PreviousHSCEmail = $null

Start-Transcript "2021-03-10-FacultySessionTranscript.txt"
...
foreach ($Resident in $Residents)
{
	...
	if ($Resident.HSCEmail -eq $PreviousHSCEmail) {
		...
		Write-Output "Hiding user from address book"
		$ADUser |
			Set-ADUser -Replace @{msExchHideFromAddressLists=$true} -ErrorAction Stop
	}
