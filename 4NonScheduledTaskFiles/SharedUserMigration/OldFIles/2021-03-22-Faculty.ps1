$Residents = Import-Csv 2021-03-22-Faculty.csv
[string]$PreviousHSCEmail = $null

Start-Transcript "2021-03-22-FacultySessionTranscript.txt"
...
foreach ($Resident in $Residents)
{
    ...
    if (($ADUser | Measure).Count -eq 1)
    {
        ...
        elseif ($AddressToApply -like "*sip*"){
            $AddressToApply = $AddressToApply
