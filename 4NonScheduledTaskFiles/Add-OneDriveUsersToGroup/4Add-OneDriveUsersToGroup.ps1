...
param (
	...
	[string]$PathToConnectionFile = "c:\ad-development\Connect-ToOffice365-MS4.ps1",
...
#Verify connection to the HSC Office 365 tenant-----------------------------
...
if ($TenantName -eq "WVUHSC")
{
	...
	$OneDriveUsers = $LogFilePath + "\" + (Get-FileDatePrefix) +  "-4Add-OneDriveUsersToGroup-Output.txt"	
	
	New-Item $ErrorLogFile -type file -force
	New-Item $OneDriveUsers -type file -force
		
	Write-LogFileSummaryInformation -FilePath $ErrorLogFile -Summary "4Add-OneDriveUsersToGroup error log"
	Write-LogFileSummaryInformation -FilePath $OneDriveUsers -Summary "Users who have been added to OneDrive"
}	
...
while ($true)
{
	...
	foreach ($user in get-content $FilePath)
	{
		...
		Add-Content -path $OneDriveUsers -value $user
		Add-Content -path $OneDriveUsers -value $Date
		Add-Content -path $OneDriveUsers -value "***************************"
		...
		{
			./Connect-ToOffice365-MS4.ps1
		}
		
		echo "***************************"
	}

	"Starting 60 second delay"
	Get-Date -format d
	Start-Sleep -s 60
}
